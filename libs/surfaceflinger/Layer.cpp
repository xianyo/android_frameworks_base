        ...
        const Transform& planeTransform, Region& outDirtyRegion)
{
    ...
    if (!dirtyRegion.isEmpty()) {
        ...
        int listLength = mDirtyRegList[mFrontBufferIndex].mDirtyRegListLength;
        for(int j = 0; j< listLength; j++)
        {
            if ( mDirtyRegList[mFrontBufferIndex].GetDiryRegionNode(j,tmpNode) > 0 )
            {
...
{
    CurrentDirtyRegList = &(mDirtyRegList[mFrontBufferIndex]);
}
