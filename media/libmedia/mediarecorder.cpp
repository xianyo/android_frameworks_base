...
status_t MediaRecorder::setAudioSampleRate(int samples_per_second)
{
    ...
    if (!(mCurrentState & MEDIA_RECORDER_DATASOURCE_CONFIGURED)) {
        LOGE("setAudioSampleRate called in an invalid state: %d", mCurrentState);
        return INVALID_OPERATION;
    }
    if (!mIsAudioSourceSet) {
        LOGE("try to set aduio sample rate without setting audio source first");
        return INVALID_OPERATION;
    }

    status_t ret = mMediaRecorder->setAudioSampleRate(samples_per_second);
    if (OK != ret) {
        LOGE("setAudioSampleRate failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
    }
    return ret;
...
status_t MediaRecorder::setAudioBitRate(int bits_per_second)
{
    ...
    if (!mIsAudioSourceSet) {
        LOGE("try to set encoded bit rate without setting audio source first");
        return INVALID_OPERATION;
    }

    status_t ret = mMediaRecorder->setAudioBitRate(bits_per_second);
    if (OK != ret) {
        LOGE("setAudioBitRate failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
    }
    return ret;
...
status_t MediaRecorder::setAudioChannel(int channel)
{
    ...
    status_t ret = mMediaRecorder->setAudioChannel(channel);
    if (OK != ret) {
        LOGE("setAudioChannel failed: %d", ret);
        mCurrentState = MEDIA_RECORDER_ERROR;
        return ret;
    }
    return ret;
